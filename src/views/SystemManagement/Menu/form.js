...
import { Form, Row, Col, Input, Radio, Select, TreeSelect } from 'antd';
  ...
  handleNextStep = (callback) => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        // 类型为代码时，Name取代码节点下拉框名称
        if(values.Category === 3) {
          data.EntryIDValue = values.EntryIDValue.value;
          data.Name = values.EntryIDValue.label;
        }
    ...
    return (
      <Form>
        <Row>
          ...
          <Col span={12}>
            {
              getFieldValue('Category') === 3 &&
              <FormItem
                {...formItemLayout}
                label="代码节点"
              >
                {getFieldDecorator('EntryIDValue', {
                  rules: [{
                    required: true, message: '请选择代码',
                  }],
                })(
                  <TreeSelect
                    dropdownStyle={{ maxHeight: 400, overflow: 'auto' }}
                    treeData={menu.codeTreeList.slice()}
                    placeholder="请选择"
