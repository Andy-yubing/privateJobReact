...
class GlobalStore {
  ...
  @computed
  get menuMap() {
    const getMenuMap = (menu, path = '', list = [], map = new Map()) => {
      menu.forEach(item => {
        if(item.hasChildren && item.children && item.children.length > 0) {
          getMenuMap(item.children, `${path}/${item.path}`, [...list, item], map);
        } else {
          let key = `${path}/${item.path}`.substr(1);
          if(item.category === 3) {
            key = `SystemManagement/Code/${item.extendValue}`;
          }
